"""
BERT 분류 모델 모듈
기존 BERT 모델을 로드하고 분류 헤드를 추가하여 MRPC 태스크에 맞게 조정
"""
...
class BERTForSequenceClassification(nn.Module):
    """BERT 기반 시퀀스 분류 모델 (MRPC용 2-class)"""
    ...
    def __init__(self, 
        ...
        Args:
            ...
            num_labels: 분류할 레이블 수 (MRPC: 2개)
            dropout: 드롭아웃 확률
            hidden_size: 숨겨진 차원 크기 (None이면 BERT의 hidden 사용)
            class_weights: 클래스 불균형 처리를 위한 가중치
        """
        super().__init__()
...
def create_classification_model(model_path: str,
                              ...
                              attn_heads: int = 12,
                              dropout: float = 0.1,
                              class_weights: Optional[torch.Tensor] = None) -> BERTForSequenceClassification:
    """분류 모델 생성 (MRPC용 2-class)"""
...
def calculate_class_weights(train_labels: list, device: str = 'cpu') -> torch.Tensor:
    ...
    # 역빈도 가중치 계산
    weights = []
    for i in range(max(label_counts.keys()) + 1):
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 더미 입력으로 테스트 (MRPC는 두 문장 입력)
        batch_size = 2
        seq_len = 128  # MRPC는 상대적으로 짧은 시퀀스
        ...
        labels = torch.randint(0, 2, (batch_size,))  # 0: not_paraphrase, 1: paraphrase
        ...
        # 클래스 가중치 테스트
        print(f"\n클래스 가중치 테스트:")
        dummy_labels = [0] * 100 + [1] * 300  # 불균형 데이터 시뮬레이션
        class_weights = calculate_class_weights(dummy_labels)
        print(f"  클래스 가중치: {class_weights}")
        
        # 가중치가 적용된 모델 생성
        weighted_model = create_classification_model(
            model_path=model_path,
            num_labels=2,
            class_weights=class_weights
        )
        print(f"  가중치 적용된 모델 생성 완료")
