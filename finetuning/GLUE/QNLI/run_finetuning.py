...
def config_to_args(config: dict):
    ...
    args.data_dir = data_config.get('data_dir', './data/QNLI')
    args.max_length = data_config.get('max_length', 512)
    ...
    args.save_dir = dir_config.get('save_dir', './finetuning_results/QNLI/checkpoints')
    args.log_dir = dir_config.get('log_dir', './finetuning_results/QNLI/logs')
    args.eval_dir = dir_config.get('eval_dir', './finetuning_results/QNLI/evaluation')
    ...
    args.wandb_project = misc_config.get('wandb_project', 'bert-qnli-finetuning')
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='BERT QNLI 파인튜닝')
...
def train_model(args, logger):
    """모델 훈련"""
    logger.info("=" * 50)
    logger.info("BERT QNLI 파인튜닝 훈련 시작")
    logger.info("=" * 50)
...
def evaluate_model_main(args, logger, checkpoint_path=None):
    """모델 평가"""
    logger.info("=" * 50)
    logger.info("BERT QNLI 모델 평가 시작")
    logger.info("=" * 50)
