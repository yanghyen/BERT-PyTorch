"""
BERT 분류 모델 모듈
기존 BERT 모델을 로드하고 분류 헤드를 추가하여 QNLI 태스크에 맞게 조정
"""
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 더미 입력으로 테스트 (QNLI는 더 긴 시퀀스)
        batch_size = 2
        seq_len = 512
        
        input_ids = torch.randint(0, 30522, (batch_size, seq_len))
        token_type_ids = torch.zeros(batch_size, seq_len, dtype=torch.long)
        # QNLI의 경우 질문과 문장을 구분하기 위해 token_type_ids 설정
        token_type_ids[:, 100:] = 1  # 100번째 토큰부터는 문장으로 가정
