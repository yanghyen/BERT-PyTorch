...
from dataset import create_data_loaders, create_matched_mismatched_loaders, get_num_labels
from finetuning_model import create_classification_model
from train import BERTTrainer
from evaluate import evaluate_model, evaluate_matched_mismatched, plot_confusion_matrix, analyze_predictions
...
def config_to_args(config: dict):
    ...
    args.data_dir = data_config.get('data_dir', './data/MNLI')
    args.max_length = data_config.get('max_length', 256)
    ...
    args.save_dir = dir_config.get('save_dir', './finetuning_results/MNLI/checkpoints')
    args.log_dir = dir_config.get('log_dir', './finetuning_results/MNLI/logs')
    args.eval_dir = dir_config.get('eval_dir', './finetuning_results/MNLI/evaluation')
    ...
    args.wandb_project = misc_config.get('wandb_project', 'bert-mnli-finetuning')
    
    # MNLI 특화 설정
    mnli_config = config.get('mnli_specific', {})
    args.use_matched_dev = mnli_config.get('use_matched_dev', True)
    args.use_mismatched_dev = mnli_config.get('use_mismatched_dev', True)
    args.evaluate_both_dev = args.use_matched_dev and args.use_mismatched_dev
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='BERT MNLI 파인튜닝')
    ...
    parser.add_argument('--evaluate_both_dev', action='store_true', default=None,
                       help='matched와 mismatched dev 모두 평가')
...
def train_model(args, logger):
    """모델 훈련"""
    logger.info("=" * 50)
    logger.info("BERT MNLI 파인튜닝 훈련 시작")
    logger.info("=" * 50)
    ...
    if args.evaluate_both_dev:
        ...
        train_loader, val_loader, test_loader = create_data_loaders(
            ...
            max_length=args.max_length,
            num_workers=args.num_workers,
            use_matched=args.use_matched_dev
        )
        val_mismatched_loader = None
    
    logger.info(f"훈련 샘플 수: {len(train_loader.dataset)}")
    logger.info(f"검증 샘플 수 (matched): {len(val_loader.dataset)}")
    if args.evaluate_both_dev:
        logger.info(f"검증 샘플 수 (mismatched): {len(val_mismatched_loader.dataset)}")
    
    # 모델 생성
    logger.info("모델 생성 중...")
    model = create_classification_model(
        model_path=args.model_path,
        num_labels=get_num_labels(),  # MNLI: 3개 클래스
    ...
    trainer = BERTTrainer(
        ...
        val_mismatched_loader=val_mismatched_loader if args.evaluate_both_dev else None
    )
    ...
    logger.info(f"최고 검증 정확도 (matched): {max(history['val_matched_accuracy']):.4f}")
    logger.info(f"최고 검증 F1 (matched): {max(history['val_matched_f1']):.4f}")
    
    if history['val_mismatched_accuracy']:
        logger.info(f"최고 검증 정확도 (mismatched): {max(history['val_mismatched_accuracy']):.4f}")
        logger.info(f"최고 검증 F1 (mismatched): {max(history['val_mismatched_f1']):.4f}")
    
    # 훈련 결과 저장
    results = {
        'args': vars(args),
        'training_time': str(training_time),
        'best_val_matched_accuracy': max(history['val_matched_accuracy']),
        'best_val_matched_f1': max(history['val_matched_f1']),
        'history': history
    }
    
    if history['val_mismatched_accuracy']:
        results['best_val_mismatched_accuracy'] = max(history['val_mismatched_accuracy'])
        results['best_val_mismatched_f1'] = max(history['val_mismatched_f1'])
...
def evaluate_model_main(args, logger, checkpoint_path=None):
    """모델 평가"""
    logger.info("=" * 50)
    logger.info("BERT MNLI 모델 평가 시작")
    logger.info("=" * 50)
    ...
    if args.evaluate_both_dev and val_mismatched_loader is not None:
        # Matched와 Mismatched 모두 평가
        logger.info("Matched와 Mismatched 데이터 모두 평가 중...")
        comparison = evaluate_matched_mismatched(
            model, val_matched_loader, val_mismatched_loader, device, args.eval_dir
        )
        
        logger.info("평가 결과:")
        logger.info(f"  Matched 정확도: {comparison['matched']['metrics']['accuracy']:.4f}")
        logger.info(f"  Matched F1: {comparison['matched']['metrics']['f1']:.4f}")
        ...
        val_results = evaluate_model(model, val_matched_loader, device)
